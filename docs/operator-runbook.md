...
sudo /opt/signage/scripts/provision-pi.sh <api-host:port>
```

`SIGNAGE_API_URL` accepts a host:port string (no scheme, no trailing slash). Example: `192.168.1.100:80` or `kpi.company.internal:443`.
...
| 1 | Missing required arguments or not running as root | Forgot `sudo` or `SIGNAGE_API_URL` |
...
| Token | Replaced with |
|-------|--------------|
| `__SIGNAGE_API_URL__` | Value of `SIGNAGE_API_URL` argument |
...
```bash
sudo SIGNAGE_API_URL=<new-api-host:port> /opt/signage/scripts/provision-pi.sh
```
