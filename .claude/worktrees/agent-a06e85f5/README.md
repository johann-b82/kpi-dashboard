...
| `https://wiki.internal`  | Outline wiki — KPI Dashboard documentation    | Phase 29  |
| `https://auth.internal`  | Dex OIDC — shared identity for both apps     | Phase 27  |

### Documentation

Developer + operator runbook: [docs/setup.md](docs/setup.md). Architecture and planning history live in `.planning/`.

- **Dex IdP** at `https://auth.internal/dex` — shared SSO for KPI Dashboard and Outline. Seeded users: `admin@acm.local`, `dev@acm.local` (placeholder passwords flagged for rotation in `dex/config.yaml`). Bcrypt user-add workflow in `docs/setup.md` "Dex first-login".
- **Outline wiki** at `https://wiki.internal` — "KPI Dashboard" collection contains 8 authored docs (Dev Setup, Architecture, API Reference, Personio Sync, Sales + HR User Guides, Settings, Admin Runbook) + a Landing page with TOC. Markdown snapshots committed to [docs/wiki-seed/](docs/wiki-seed/).
- **Known limitation:** Dex `staticPasswords` connector does not set a cross-app browser session; each OIDC client re-prompts for the password even in the same browser. Value delivered = shared credentials, not one-click SSO. See `docs/setup.md` "Known limitations".
...
docker compose up
  ...
  +-- dex            (ghcr.io/dexidp/dex:v2.43.0)          --> internal :5556
  +-- outline-db     (postgres:17-alpine)                  --> internal :5432 (separate volume)
  +-- outline-redis  (redis:7-alpine)                      --> internal :6379
  +-- outline        (outlinewiki/outline:0.86.0)          --> internal :3000
...
Boot order is healthcheck-gated: every service waits for its dependencies to be healthy before starting, which closes the 502 window on cold boot. All 9 services run from a single `docker-compose.yml`.
...
|---------|------|-------------|
| v1.11 | 2026-04-15 | Outline Wiki + Shared Auth (Dex) — NPM edge TLS, Dex OIDC IdP, KPI Dashboard SSO, Outline wiki at wiki.internal, NavBar wiki icon, KPI Light → KPI Dashboard rebrand, 9-page docs collection |
