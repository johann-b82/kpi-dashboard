...
**Shipped:** v1.11 complete (2026-04-15) — Dex OIDC IdP + KPI Dashboard SSO + Outline wiki + 9-page KPI Dashboard documentation collection with Mermaid architecture diagram and admin runbook. Phase 30.1 rebranded product from KPI Light → KPI Dashboard (display only). Known limitation: Dex staticPasswords re-prompts per OIDC client (documented; shared credentials, not one-click SSO).
