...
- **47/47 requirements landed.** All SGN-DB/BE/SCH/ADM/PLY/DIFF/OPS/INF IDs have code + tests + doc evidence in the v1.16 phase directories.
- **SGN-OPS-01 path amendment:** text said `frontend/src/docs/admin/digital-signage.{en,de}.md`; actual implementation files live at `frontend/src/docs/{en,de}/admin-guide/digital-signage.md` to match repo convention. See `48-VERIFICATION.md §Requirement amendments` for full rationale.
- **SGN-OPS-03 E2E walkthrough carry-forward:** code-complete (security checks verified via committed systemd units + sidecar source). Real-hardware walkthrough (`48-E2E-RESULTS.md`) is deferred to operator run-time — the 5-scenario scaffold is ready to fill in-place. Does not block milestone closure.
- **SGN-PLY-05 ownership:** heartbeat lives in the Pi sidecar's `_heartbeat_loop` (Plan 48-01), not the kiosk browser.
- **Phase 47 defects closed inside v1.16:** D-7 (SW scope) resolved by sidecar replacing runtime-cache path; D-8 (`cache: no-store`) applied in Plan 48-03; G2 (bundle cap) raised 200 000 → 210 000 gz per orchestrator decision option-A.
...
- [x] **SGN-PLY-05**: Heartbeat — `POST /api/signage/player/heartbeat` every 60s with `current_item_id`
