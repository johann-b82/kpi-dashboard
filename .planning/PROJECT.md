...
**In progress:** v1.18 Phase 50 (Pi Polish) complete 2026-04-21 — player entry bundle split via dynamic-import of `PdfPlayer` + `react-pdf`/`pdfjs-dist` (204 KB gz → 75 KB gz entry, lazy chunks ~126 KB gz), guard `LIMIT` reset to 200 000 with per-file entry-vs-lazy measurement; Scenarios 4 + 5 operator walkthrough signed off on a `provision-pi.sh`-provisioned Bookworm Lite 64-bit Pi (thresholds met by direct observation; exact timings not captured — honest deviation recorded in `50-E2E-RESULTS.md`). SGN-POL-04 + SGN-POL-05 closed.
