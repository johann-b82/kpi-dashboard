...
**In progress:** v1.18 Phase 51 (Schedule schema + resolver) complete 2026-04-21 — Alembic v1_18 migration adds `signage_schedules` (weekday_mask + start_hhmm/end_hhmm + playlist_id + priority + enabled) and `app_settings.timezone` with CHECK constraints + partial index; new `resolve_schedule_for_device` service composes into the existing playlist resolver (timezone-aware via `zoneinfo`/`tzdata`, parameterized SQL bitmask match, priority + updated_at ordering); admin CRUD router `/signage_admin/schedules` with post-commit SSE `schedule-changed` fanout + playlist DELETE 409 on FK RESTRICT. SGN-TIME-01..04 closed. Phase 50 (Pi Polish) previously closed SGN-POL-04/05.
...
---
*Last updated: 2026-04-21 — Phase 50 (Pi Polish) complete — player entry bundle dynamic-split (204 KB gz → 75 KB gz, `PdfPlayer` + `react-pdf` lazy), guard LIMIT reset to 200 000 with entry-vs-lazy measurement; Scenarios 4 + 5 hardware E2E signed off on a `provision-pi.sh`-provisioned Bookworm Lite 64-bit Pi (thresholds met by observation, numerical timings not captured — honest deviation in `50-E2E-RESULTS.md`). SGN-POL-04, SGN-POL-05 closed (2/2 plans).*
