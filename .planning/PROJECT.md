...
**In progress:** v1.11 Outline Wiki + Shared Auth (Dex) — Phase 29 complete (Outline 0.86.0 live at https://wiki.internal with own Postgres/Redis, local file storage, Dex OIDC with split browser/internal endpoints, mkcert CA trust, BSL 1.1 compliance note). Known limitation: Dex staticPasswords does not provide silent cross-app SSO — documented in docs/setup.md; shared credentials delivered, not one-click SSO.
