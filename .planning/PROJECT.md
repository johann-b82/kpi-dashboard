...
**In progress:** v1.11 Outline Wiki + Shared Auth (Dex) — Phase 30 complete (NavBar now has a BookOpen icon opening https://wiki.internal in a new tab, positioned between LanguageToggle and Upload, with translated `nav.wiki` tooltip and `VITE_WIKI_URL` env override). Carried-forward limitation from Phase 29: Dex staticPasswords does not provide silent cross-app SSO — shared credentials delivered, not one-click SSO.
...
| Dex over Authentik/Authelia for v1.11 IdP | Config-file driven, single Go binary, no admin UI to manage — matches internal-tool scale | ✓ v1.11 Phase 27 |
| bcrypt hashes via `python:3.12-alpine` + `bcrypt` lib | Dex v2.43.0 removed the `hash-password` subcommand; canonical workflow now runs a throwaway python container | ✓ v1.11 Phase 27 |
| `user: root` on dex service | Dex image's UID 1001 cannot write to root-owned `/data` named volume; simpler than chown-init sidecar at internal-tool scale | ✓ v1.11 Phase 27 (revisit if hardening required) |
