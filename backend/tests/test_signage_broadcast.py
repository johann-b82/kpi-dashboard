...
Covers decisions:
  - D-03: last-writer-wins on duplicate subscribe(device_id)
...
async def test_subscribe_creates_queue_with_maxsize_32():
    ...
    assert q.maxsize == 32
    assert signage_broadcast._device_queues[1] is q


# --------------------------------------------------------------------------
# B: D-03 last-writer-wins — a second subscribe replaces the prior queue
# --------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_subscribe_replaces_existing_queue_last_writer_wins():
    q1 = signage_broadcast.subscribe(1)
    q2 = signage_broadcast.subscribe(1)
    assert q1 is not q2
    assert signage_broadcast._device_queues[1] is q2
...
# --------------------------------------------------------------------------
# D: notify_device enqueues the payload on the subscriber's queue
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# G: D-03 + Pitfall 7 — re-subscribe resets the warn-once flag (new queue)
# --------------------------------------------------------------------------
...
async def test_warned_flag_reset_on_resubscribe(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="app.services.signage_broadcast"):
        # First overflow — first warn fires.
        signage_broadcast.notify_device(1, {"n": 100})

        # New connection: last-writer-wins replaces the queue. The new queue
        # has no `_warned_full` attr, so the next overflow must warn again.
        q2 = signage_broadcast.subscribe(1)
        assert q2 is not q1
        for i in range(32):
            q2.put_nowait({"m": i})
